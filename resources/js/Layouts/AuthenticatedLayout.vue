  ...
  <div class="md:flex md:flex-col">
    <div class="md:flex md:flex-col md:h-screen">
      <div class="md:flex md:shrink-0">
        ...
        <!-- Top Navbar -->
        <div class="md:text-md hidden md:flex items-center justify-between p-4 w-full text-sm bg-white border-b md:px-12 md:py-0">
          <div class="mr-4 mt-1"></div>
          <Dropdown class="mt-1" placement="bottom-end">
            <template #default>
              <div class="group flex items-center cursor-pointer select-none">
                <div class="mr-1 text-gray-700 group-hover:text-indigo-600">
                  <span>{{  $page.props.auth.user.name }}</span> 
                  <!-- <span class="hidden md:inline">&nbsp;{{ $page.props.auth.user.last_name }}</span> -->
                </div>
                <svg class="w-5 h-5 fill-gray-700 group-hover:fill-indigo-600 focus:fill-indigo-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M5 6l5 5 5-5H5z" /></svg>                  </div>
            </template>
            <template #dropdown>
              <div class="mt-2 py-2 text-sm bg-white rounded shadow-xl">
                <Link class="block px-6 py-2 hover:text-white hover:bg-indigo-500" :href="'/profile'">My Profile</Link>
